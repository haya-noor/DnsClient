...
export class DnsQuestion 
{
    ...
    static fromBuffer(buffer: Buffer, offset: number): DnsQuestion {
        ...
        console.log(`Parsed question for domain: ${domain}, type: ${queryType}, class: ${qclass}`);
        return new DnsQuestion(domain, queryType);
    }

    getLength(): number {
        // The length of the domain name section plus the QTYPE (2 bytes) and QCLASS (2 bytes)
    ...
    writeToBuffer(buffer: Buffer, offset: number): number {
        ...
        console.log(`Question for ${this.domain} with query type ${this.queryType} written to buffer. New offset: ${offset}`);
        return offset;
    }
